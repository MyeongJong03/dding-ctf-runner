...
`dding-ctf-runner` is a shell-first competition runner. `ctfctl` is the stable control plane for preflight, interactive board state, claims, persistent Docker containers, auth metadata checks, platform adapters, guarded submissions, and local-only postsolve output.

`~/ctf-solver` remains unchanged and can be used as a library or reference toolbox. This repo provides the competition-specific operations layer around the interactive Codex swarm and live platform policy.
...
The mode guard is enforced in the CLI, interactive helpers, and legacy worker loop. Setup and rehearsal modes block real live submit.

## Interactive Swarm

The default live workflow is interactive:

```bash
ctfctl interactive init --contest-id <id> --profile <profile> --agents 4 --json
ctfctl interactive sync --contest-id <id> --profile <profile> --live --download --ingest --json
ctfctl interactive prompt --contest-id <id> --agent agent-1
cd ~/CTF && codex
```

Every visible Codex terminal is an autonomous solver. There is no controller/solver split. Windows WSL operators can run up to six Codex terminals; MacBook operators should start with four.

Interactive state lives under `~/CTF/contests/<contest>/operator/`, outside git. The directory stores `BOARD.md`, `board.json`, `claims/`, `memos/`, accepted solve records, external solved markers, and stalled handoffs. Claim locks prevent same-machine duplicate claims by default; `--allow-duplicate` is explicit and local-only. Cross-machine duplicate coordination is intentionally manual.

Interactive writeups are accepted-only and are generated as Korean and English files named `[category]ChallengeNameWriteup.ko.md` and `[category]ChallengeNameWriteup.en.md`. Unsolved challenges keep memos and stalled records only.

## Legacy Contest Control Plane

The contest arm/supervisor control plane remains for legacy/advanced background automation. It is not the default live workflow. Control state lives outside git at:
...
The legacy advanced arm sequence is explicit:
...
Legacy Codex workers are isolated from the user's global Codex profile. Plain `codex` is correct for the default interactive swarm from `~/CTF`, but legacy background workers must use `scripts/ctf-worker-*` wrappers instead. The wrappers call `scripts/ctf-worker`, then `scripts/run-codex-worker.sh`; this execution model changes to `~/dding-ctf-runner`, sets `CODEX_HOME=~/.codex-workers/<worker-id>`, adds the runner-safe writable directories with de-duplication, and relies on the runner's slim `AGENTS.md`.
...
Legacy competition workers default to Codex model auto-selection: the wrapper omits `--model` unless `CTF_CODEX_MODEL` is set to a concrete non-`auto` value. Auto/unpinned means "use the installed Codex CLI's current default model"; it does not promise the newest or strongest available model. The runner repo intentionally does not hard-code a default model. Worker-local `config.toml` also stays unpinned by default; `ctfctl codex set-model` is for reproducibility, and `ctfctl codex unset-model` / `unset-model-all` returns workers to auto/unpinned policy. A hard-pinned model is a preflight warning, not a blocker.

Legacy competition workers also default to no-prompt automation: `--ask-for-approval never` plus `--sandbox danger-full-access`. Operators can opt down with `CTF_CODEX_DANGER=0` and an explicit `CTF_CODEX_SANDBOX` such as `workspace-write`. `CTF_CODEX_IGNORE_USER_CONFIG=1` isolates the launch through the worker home without modifying `~/CTF/AGENTS.md`, `~/CTF/CLAUDE.md`, `~/.codex/AGENTS.md`, or raw `~/.codex/config.toml`.

Legacy wrapper launches:
...
Plain `codex` is the default for interactive CTF solving from `~/CTF`. Use `ctf-worker-*` only for legacy worker/supervisor automation.
