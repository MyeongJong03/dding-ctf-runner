...
summary = {
    ...
    "fake_full_rehearsal_command": commands.get("mock_full_rehearsal"),
}
print(summary)
required_commands = {"compileall", "pytest", "preflight", "mock_full_rehearsal", "codex_mini_rehearsal", "fresh_clone_check", "history_scan"}
missing_command_summaries = sorted(required_commands - set(commands))
if data.get("status") != "ok" or not uv_lock.get("policy_ok") or missing_command_summaries:
    raise SystemExit(1)
PY
